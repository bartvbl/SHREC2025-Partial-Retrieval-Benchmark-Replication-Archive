...
namespace ShapeBench {
 ...
 void computeDescriptors(
            ...
            std::vector<DescriptorType>& referenceDescriptors) {
        ...
        std::vector<bool> allDescriptorsActive(numberOfSupportRadiiToTry, true);
        ...
        #pragma omp parallel for default(none) shared(allDescriptorsActive, startTime, representativeSetSize, representativeSet, fileCache, dataset, std::cout, config, referenceCountProcessed, referencePointCloudSamplingSeed, supportRadiiToTry, referenceDescriptorGenerationSeed, numberOfSupportRadiiToTry, referenceDescriptors) schedule(dynamic)
        for(uint32_t referenceIndex = 0; referenceIndex < representativeSetSize; referenceIndex++) {
            ...
            #pragma omp critical
            {
                ...
                ShapeBench::printETA(startTime, referenceCountProcessed, representativeSetSize);
                std::cout << std::flush;
            };
            ...
            ShapeBench::computeDescriptors<DescriptorMethod, DescriptorType>(
                    representativeSetMesh, representativeSetPointCloud, {orientedPoints.size(), orientedPoints.data()}, allDescriptorsActive, config,
            ...
            /*if(referenceIndex == 0) {
                ShapeDescriptor::cpu::array<ShapeDescriptor::RICIDescriptor> converted {generatedDescriptors.size(), reinterpret_cast<ShapeDescriptor::RICIDescriptor*>(generatedDescriptors.data())};
                ShapeDescriptor::writeDescriptorImages(converted, "supportradius.png", false);
            }*/
    ...
    float estimateSupportRadius(const nlohmann::json& config, const Dataset& dataset, ShapeBench::LocalDatasetCache* fileCache, uint64_t randomSeed) {
        ...
        std::vector<DescriptorType> sampleDescriptors(sampleVerticesSet.size() * numberOfSupportRadiiToTry);

                computeDescriptors<DescriptorMethod, DescriptorType>(
